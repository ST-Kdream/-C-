#include "record.h"
#include "rank.h"

//记录保存函数定义
void record_save(bool& is_win, std::string mode, int& difficulty, int& attempts, int& max_num, int& chance,int& EP) {
	time_t now = time(0);
	char time_str[26];
	ctime_s(time_str, sizeof time_str, &now);
	std::ofstream record_file("game_record.txt", std::ios::app);
	if (record_file.is_open()) {
		record_file << "游戏时间: " << time_str;
		record_file << "游戏结果：" << (is_win ? "胜利" : "失败") << "  ";
		record_file << "获得经验：" << (is_win ? EP : 0) << std::endl;
        record_file << "游戏模式：" << mode << "   ";
		record_file << "难度等级: " << difficulty << std::endl;
		record_file << "最大数字: " << max_num << std::endl;
		record_file << "猜测次数: " << attempts << "/" << chance << std::endl;
		record_file << "----------------------------------------" << std::endl;
		record_file.close();
		std::cout << "游戏记录已保存到'game_record.txt'" << std::endl;
	}
	else {
		std::cout << "无法打开记录文件，游戏记录未保存。" << std::endl;
	}
}

//玩家信息显示函数定义
bool player_information() {
   	std::ifstream player_file("player_information.txt");
	if (player_file.is_open()) {
		std::string player_line;
		std::cout << "玩家信息记录：" << std::endl;
		while (getline(player_file, player_line)) {
			std::cout << player_line << std::endl;
		}
		player_file.close();
		return true;
	}
	else {
		std::cout << "无法打开记录文件，无法显示玩家信息。" << std::endl;
		return false;
	}
}

//初始化玩家信息函数定义
void player_init(std::string name,int& go_first) {
	std::cin >> name;
	std::ofstream player_init("player_information.txt");
	if (player_init.is_open()) {
		player_init << name << std::endl;
		player_init << "总经验值:" << 0 << std::endl;
        player_init << "段位：迷雾探索者" << std::endl;
		player_init.close();
	}
	else {
		std::cout << "无法创建玩家信息文件，玩家信息未保存。" << std::endl;
	}
	std::cout << "玩家信息已保存，欢迎你，" << name << "！" << std::endl;
	go_first = 1;
	
}

//玩家经验值信息更新函数定义
int player_update(int& EP) 
{
    int sum_EP;  
    std::string EP_line;  
    std::string first_line;  
    std::string rank_line;
    // 用binary模式打开，避免换行符转换导致的乱码
    std::fstream player_file("player_information.txt", std::ios::in | std::ios::out | std::ios::binary);  

    if (player_file.is_open()) {
        // 读取第一行（玩家名）和第二行（经验值行）
        getline(player_file, first_line);
        getline(player_file, EP_line);
        getline(player_file, rank_line);

        // 查找冒号（兼容中文“：”和英文“:”）
        size_t pos = EP_line.find("：");
        if (pos == std::string::npos) {
            pos = EP_line.find(":");  // 补充英文冒号兼容
        }

        if (pos != std::string::npos) 
        {
            // 截取冒号后内容，并过滤出纯半角数字（解决格式错误核心）
            std::string num_str = EP_line.substr(pos + 1);  // 跳过冒号
            std::string pure_num;  // 临时变量，用于存储过滤后的数字
            // 用ASCII范围判断半角数字（替代isdigit，避免误判）
            for (char c : num_str) 
            {
                if (c >= '0' && c <= '9')    // 只保留0-9的半角数字
                {  
                    pure_num += c;
                }
            }

            // 检查是否提取到有效数字
            try
            {
                if (pure_num.empty())
                {
                    player_file.close();
                    throw std::runtime_error("更新玩家信息错误");
                }
            }
            catch (const std::exception& e)
            {
                std::cout << "玩家信息格式错误，未找到有效数字（仅支持半角0-9）。" << std::endl;
                player_file.close();
                exit(12);
            }
            

            // 转换数字并计算总经验值
            try {
                sum_EP = stoi(pure_num) + EP;  
            }
            catch (const std::invalid_argument&) {
                std::cout << "玩家信息格式错误，无法读取经验值。" << std::endl;
                player_file.close();
                exit(13);
            }
            catch (const std::out_of_range&) {
                std::cout << "玩家信息中的经验值超出范围，无法读取经验值。" << std::endl;
                player_file.close();;
                exit(14);
            }

            // 关闭原文件，用truncate模式重新打开写入（解决truncate成员错误）
            player_file.close();
            std::ofstream out_file("player_information.txt", std::ios::out | std::ios::trunc | std::ios::binary);
            if (out_file.is_open()) {
                // 写回玩家名和新经验值行（用\r\n避免换行符乱码）
                out_file << first_line << "\r\n";
                out_file << "总经验值：" << sum_EP << "\r\n";
                out_file << rank_line << "\r\n";
                out_file.close();
                std::cout << "经验值更新成功！当前总经验值：" << sum_EP << std::endl;
                return sum_EP;
            }
            else {
                std::cout << "无法写入文件，可能被占用。" << std::endl;
            }

        }
        else {
            // 未找到冒号的错误处理
            std::cout << "玩家信息格式错误，未找到冒号（：或:）。" << std::endl;
            player_file.close();
        }

    }
    else {
        // 文件打开失败的错误处理
        std::cout << "无法打开玩家信息文件，请检查路径或文件是否被占用。" << std::endl;
    }
}

//段位更新函数头文件
void rank_update(std::string rank_name)
{
    std::string rank_line = "玩家段位为：" + rank_name;
    std::vector <std::string> fline;
    std::ifstream in_file("player_information.txt");
    if (in_file.is_open())
    {
        std::string line;
        while (getline(in_file, line))
        {
            fline.push_back(line);
        }
        in_file.close();
    }
    else
        std::cout << "更新段位时文件打开失败！" << std::endl;

    if (fline.size() == 3)
    {
        fline[2] = rank_line;
    }
    else
        std::cout << "玩家信息格式错误！" << std::endl;

    std::ofstream out_file("player_information.txt");
    if (out_file.is_open())
    {
        for (const auto& out_line : fline)
        {
            out_file << out_line << "\r\n";
        }
        out_file.close();
        std::cout << "段位更新成功，你现在的段位是：" << rank_name << std::endl;
    }
    else
    {
        std::cout << "玩家信息文件无法打开" << std::endl;
    }
}

//显示游戏规则函数定义
void show_rules() {
	std::ifstream rule_file("game_rules.txt");
	if (rule_file.is_open()) {
		std::string rule_line;
		while (getline(rule_file, rule_line)) 
        {
			std::cout << rule_line << std::endl;
		}
		rule_file.close();
    }
	else
    {
		std::cout << "无法打开规则文件，无法显示游戏规则。" << std::endl;
	}
}

//统一更新函数定义
void update_all(int update_EP)
{
    int sum_EP = player_update(update_EP);  //调用玩家经验值信息更新函数
    std::string rank_name = Rank::update_rank(sum_EP, rankings);
    rank_update(rank_name);
}
